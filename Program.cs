...
class Program
{
    ...
    static void Main()
    {
        ...
        //var check_email = debtors.Where(d => d.Email.EndsWith("rhyta.com") || d.Email.EndsWith("dayrep.com")).ToList();

        //2) Yashi 26-dan 36-ya qeder olan borclulari cixartmag
        //var check_age = debtors.Where(d => d.BirthDay.Year <= 1997 && d.BirthDay.Year >= 1987).ToList();

        //3) Borcu 5000-den cox olmayan borclularic cixartmag
        //var check_debt = debtors.Where(d => d.Debt > 5000).ToList();

        //4) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2-den cox 7 reqemi olan borclulari cixartmaq
        //var check = debtors.Where(d => d.FullName.Length > 18 && d.Phone.Count(c => c == '7') >= 2).ToList();

        //5) Qishda anadan olan borclulari cixardmaq 
        //var check = debtors.Where(d => d.BirthDay.Month == 12 || d.BirthDay.Month <= 2).ToList();
        ...
        //var check = debtors.Select(d => d.Debt).ToList();
        //double avg = check.Average();
        //var check_mta = debtors.Where(d => d.Debt > avg).Select(d => d.FullName).ToList();
        //check_mta.ForEach(d => Console.WriteLine(d));

        //7) Telefon nomresinde 8 olmayan borclularin yalniz familyasin,yashin ve umumi borcun meblegin cixarmaq
        //var check = debtors.Where(d => d.Phone.Count(c => c == '8') == 0).Select(d => d.FullName).ToList();

        //11)Butun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq
        //var check = debtors.Select(d => d.Debt).ToList();
        //Console.WriteLine(check_ad.Sum());

        //12)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq
        //var check = debtors.Where(d => d.BirthDay.Year < 1945).Select(d => d.FullName).ToList();

        //15)Adindaki ve familyasindaki herflerden "smile" sozunu yaza bileceyimiz borclularin siyahisini cixartmaq
        //var check = debtors.Where(d => d.FullName.Count(c => c == 's') > 0 && d.FullName.Count(c => c == 'm') > 0 && d.FullName.Count(c => c == 'i') > 0 && d.FullName.Count(c => c == 'l') > 0 && d.FullName.Count(c => c == 'e') > 0).Select(d => d.FullName).ToList();
